export async function onRequestPost({ request, env, data }) {
  ...
  const record = {
    id,
    type: 'fire_safety_lighting_test',
  ...
  await env.CFR_DATA.put(`fire_safety:lighting:${date}:${id}`, JSON.stringify(record));
...
export async function onRequestGet({ request, env, data }) {
  ...
  const { keys } = await env.CFR_DATA.list({ prefix: 'fire_safety:lighting:' });
